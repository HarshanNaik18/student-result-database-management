...
class studentsClass:
    def __init__(self,root):
        ...
        lbl_curse=Label(self.root,text="Course",font=("goudy old style",15,'bold'),bg='white').place(x=360,y=180)
...
#=========================
    ...
    def add(self):
        ...
        try:
            if self.var_roll.get()=="":
                ...
                cur.execute("select * from students where name=?",(self.var_roll.get(),))
                row=cur.fetchone()
                if row!=None:
    ...
    def search(self):
        ...
        cur=con.cursor()
        try:
            cur.execute(f"select * from students where roll=?",(self.var_search.get(),))
            rows=cur.fetchone()
            if rows!=None:
                self.CourseTable.delete(*self.CourseTable.get_children())
