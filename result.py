...
class resultClass:
    def __init__(self,root):
        ...
        #===title===
        title=Label(self.root,text="Add Student Result Details",font=("goudy old style",20,"bold"),bg="orange",fg="#262626").place(x=10,y=15,width=1500,height=50)
    ...
    def search(self):
        ...
        cur=con.cursor()
        try:
            cur.execute(f"select name,course from students where roll=?",(self.var_roll.get(),))
            rows=cur.fetchone()
            if rows!=None:
